...
- Hibernate의 영속성 컨텍스든 JDBC든 같은 Datasource로부터 동일한 Connection을 발급받기 때문에, 서비스 로직에 이 둘을 동시에 처리하여도
  동일한 Connection을 <code>TransactionSynchronizationManager</code>에 동기화 시킨다. 그래서 JPA와 JdbcTemplate를 동시에 사용하여도
  <code>@Transactional</code>안에 있다면 동일한 범주안에서 커밋되거나 롤백이된다.

- Redis의 저장할 데이터를 선별할 때 동시성과 데이터 유실 민감도를 고려하여야한다. 가령, 게시글은 작성 빈도가 낮고 동시성 충돌이 없으며 유실 시 치명적이기 때문에
  즉각적으로 RDBMS에 영속화해야한다. 반면 '좋아요'와 같이 갱신 빈도가 높고 동시성 충돌이 많으면 단일 ROW에 수만 건 쓰기(UPDATE) 요청이 동시 다발적으로 몰려
  DB 락(Lock)경합과 데드락을 유발하는 원인이 된다. 따라서 Redis에 메모리에 ID와 카운트 같은 가벼운 데이터만 적재하여 O(1)의 속도로 동시서을 제어하고,
  무거운 게시글 본문 데이터는 DB로 직행시키는 관심사분리(Separation of Concerns) 아키텍처를 적용하는것을 고려해보는것이 좋다.

### 문제 해결

- 스프링에서 @Transactional을 사용하면, 현재 스레드 전용 사물함인 ThreadLocal에 DB 커넥션이 보관된다. 이 사물함은 해당 스레드만 열어볼 수 있다.
  따라서 테스트 코드(메인 스레드)에서 회원을 등록했더라도, 파생된 작업 스레드들은 메인 스레드의 커넥션(사물함)에 접근할 수 없어 새로운 커넥션을 맺게 된다.
  결과적으로 작업 스레드들 입장에서는 메인 스레드가 아직 '커밋(Commit)하지 않은 임시 데이터'를 읽을 수 없으므로(트랜잭션 격리), 
  조회 시 데이터가 없다고 판단하여 좋아요 값이 0이 나오는 문제가 발생했다. 그래서 @Transactional을 제거하고, 수동으로 롤백하였다.

- Look-Aside 캐시 로직 단위 테스트 중, 동일한 add 메서드가 다른 인자(캐시 복구용, 중복 검사용)로 호출 될 때, Mockito의 엄격한 검증 모드(Strict Stubbing)가
  이를 인자 매칭 오류(PotentialStubbingProblem)로 간주하여 테스트가 실패하는 이슈가 발생하였다. 검증을 회피하는 <code>lenient()</code> 설정 대신, 캐시 히트/미스
  상황에 따라 인자(Argument)를 정확하게 분석하였고, 캐시 복구, 중복 방지, 터티 체킹용 add 로직 각각에 대한 <code>given</code>절을 정확한 변수로 분리하여 타중 스터빙(Multiple Stubbing)을 구현하였다.
...
- 톰캣의 로직을 처리하는 활성화하는 최대 스레드를 설정(max-threads)할 수 있는데, 이 값은 기본적으로 200이다. 그리고 활성 스레드가 최대였을 때,
  대기하는 요청들을 저장할 수 있는 최대 수를 설정(accept-count)할 수도 있는데, 보통 100이다. 만약, 대기 요청 최대수를 넘어가는 요청이 더 들어오면,
  요청을 바로 튕겨 내기때문에 에러 로그 조차 남기지 않는다.

- 스프링 JPA에서 @Column(unique=true)를 걸어주면, 중복방지와 인덱스생성이 모두 가능해진다. 이거 덕분에, 동시에 가입을 하려고하면 나중에 가입되는 것은
  튕겨낸다.


### 문제 해결

- AWS에 웹 애플리케이션을 띄워서, JMeter로 테스트를 해보았다. 그러나, 50000개의 회원가입부터 막혔고, 원인은 DB connection pool이 부족하여서
  DB에 적재를 못해서 였다. 그래서, Hikari Pool의 DB Connection의 수를 100개로 늘렸다. 그리고 1000개의 회원가입은 통과했다. AWS EC2(Free-Tier)로 JMeter테스트를 해보았다.
  결과는 다음과 같다.

    | Label | #Sample | Average | Min | Max | Std.Dev. | Error% | Throughtput | Received KB/sec | Sent KB/sec | Avg.Bytes |
    |---| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
    | 회원가입 | 999 |1807|265|3886|840.9607196290357|0.0|16.609583347188508|6.5188542515296115|5.1546445316396765|401.8948948948949|

  회원가입시 보통 500ms가 권장된다고하는데, 나의 평균 시간은 1897ms로 3배나 느리다. 최대 응답 시간도 너무 많이 나온다고 한다. 그리고 표준변차 응답 시간의 변동폭이
  너무 크고, 처리량이 초당 5개로, 초당 5명만 가입이 가능하다. 중복확인하는 이메일과 닉네임에는 제대로 인덱스가 걸려있으니, 아마 FreeTier의 한계일 확률이 높다.
  특히, BCrypt로 암호화하는데 CPU를 많이 잡아먹을 확륭이 크다고한다. 그리고 4000개의 동시 회원가입은 처리하지 못하고, 오류가 떴다.
  코드로 해결을 보기 힘들고, 서버를 스케일아웃해야하나 지금은 그럴 수 없으니 처리 시간을 길게 잡는게 맞는거 같다.

- 처리 시간을 길게 잡아도 해결이 되지 않는다. 그 이유는 20초마다 1000명당 회원가입 처리량이 16개였고, 톰캣의 스레드는 200개, 대기열은 500개 이다.
  10000개의 회원가입 처리하고자 할 때, 1초에 50명씩 들어갈것이다. 그런데, 서버는 1초에 16명밖에 처리를 하지 못한다. 그래서 25초 정도가 되면, 서버가
  처리를 감당하지 못했다. 그래서, 테스트할 회원들은 서버에 넣고 로그인 시켜야할 거같다. 그런데 Free-Tier의 1GB메모리는 이 회원들의 로그인의 세션을 감당하지 못할 것이다.
  그래서 로그인을 세션에서 JWT 토큰방식으로 바꿔야할거같다.
...
- 회원 50000명을 적제하고 로그인 시키기위해서 JWT 도입하기
...
- OncePerRequestFilter는 한 번의 요청에 한 번의 필터를 적용한다는 건데, 포워드 시에 또 필터를 타는걸 방지하기 위해서이다.
- XSS는 페이지에 자바스크립트 코드를 심어놔서 쿠키의 세션이나 로컬 스토리지의 데이터를 탈취하는 공격 기법이다.
- CSRF는 가짜로 만든 더미 페이지에서 브라우저의 쿠키로 권한을 도용해, 실제 서버에 원치 않는 요청을 하는 기법이다.
- 그러나 JWT는 쿠키를 사용하지 않기 때문에, CSRF의 공격을 무력화 하고 인가를 통과 할 수 있다. 그러나 XSS는 토큰이 로컬 스토리지나 쿠키에 저장할 경우 탈취당할 수 있다.
- 스프링 시큐리티는 DelegatingFilterProxy를 톰캣의 필터에 등록해서, 톰캣과 스프링 컨테이너의 브릿지 역할을 하도록 한다.
- UsernamePasswordAuthenticationFilter와 내가 등록한 JwtAuthenticationFilter는 시큐리티 필터 체인의 필터이다. 즉, DelegatingFilterProxy가 아니다.

### 문제 해결

- JWT 도입 및 무상태성 아키텍처 전환
...
- 테스트 코드에서 @AutoConfigureMockMvc(addFilters = false)는 DelegatingFilterProxy의 필터를 거치지 않도록 설정한다.
- 테스트 코드에서 @WithMockUser는 SecurityContextHolder에 해당하는 username과 roles를 넣어준다. 이렇게 하는 이유는 테스트 코드에서,
  SecurityContextHolder를 뒤지는 로직이 있을지 모르기 때문이다.
- HandlerMethodArgumentResolver라는 인터페이스를 상속하여서 컨트롤러의 메서드 파라미터를 조립할 수 있는데,
  andlerMethod(컨트롤러 메서드: @GetMapping, @PostMapping)을 뜻하고, ArgumentResolver는 파라미터를 조립해주는 객체이다.
  즉, 컨트롤러의 메서드들의 파라미터들을 supportsParameter로 체크하여서, resolveArgument로 객체를 조립한다.

### 문제 해결

#### 1. JWT Stateless 아키텍처 전환에 따른 유저 식별자 주입 문제 해결

- 문제 상황: 톰캣 세션을 제거하고 JWT를 도입하면서, 기존 컨트롤러들이 의존하던 세션 기반의 유저 정보(MemberDto)를 더 이상 가져올 수 없는 문제 발생.

- 해결 과정:

   토큰 검증 필터(JwtAuthenticationFilter)를 구현하여, 인증된 유저 식별자를 스레드 로컬인 SecurityContextHolder에 임시 저장하도록 아키텍처 변경.

   스프링 MVC의 HandlerMethodArgumentResolver를 활용한 커스텀 어노테이션 구현. 컨트롤러 파라미터 매핑 시 시큐리티 컨텍스트를 조회하고 DB에서 DTO를 조립하여 자동으로 주입.

- 결과: 컨트롤러 내부의 비즈니스 로직을 전혀 수정하지 않고도 무상태 환경에서 유저 정보를 우아하게 주입하는 환경 구축 완료.

#### 2. 시큐리티 도입에 따른 슬라이스 테스트(Slice Test) 환경 재구축

- 문제 상황: JWT 필터가 추가되면서 기존 컨트롤러 단위 테스트(@WebMvcTest)들이 인증 과정에 막혀 전부 실패함.

- 해결 과정:

  @AutoConfigureMockMvc(addFilters = false)를 설정하여 DelegatingFilterProxy의 개입을 차단, 불필요한 필터 로직을 우회하여 테스트 격리성 확보.

  @WithMockUser를 사용하여 테스트 실행 직전 SecurityContextHolder에 인증 객체를 미리 주입함으로써 정상적인 토큰 검증을 통과한 것과 동일한 환경 모킹(Mocking).

#### 3. Docker 배포 시 환경 변수 주입 이슈 해결

- 문제/해결: Docker 컨테이너 실행 시 환경 변수(env)를 넘기더라도 Java 애플리케이션(jar) 실행 명령어에 명시적으로 전달되지 않는 현상 파악 및 수정 완료.
...
- **톰캣 스레드와 히카리 커넥션 풀 최적화하기**
    - Hikari CP의 `max-lifetime`, `connection-timeout`, `minimum-idle`, `maximum-pool-size`의 값들과 Tomcat의 threads의 `max`, `accept-count` 값들을 조정합니다.
    - Hikari CP의 `max-lifetime`은 DB 소켓과의 최대 연결 유지 시간을 의미합니다. 이 값의 조정이 왜 필요하냐면, DB의 처리 요청 대기 시간(`wait_timeout`), OS(특히, 리눅스)나 라우터, 방화벽 같은 장비들은 중간에서 통신을 하지 않으면 자원 관리를 위해 소켓을 강제로 닫아 버리기 때문입니다.
    - 만약 RDS의 처리 요청 대기시간(`wait_timeout`)이 60초인데 Hikari CP의 `max-lifetime`이 9분 40초(580000ms)라면, Hikari CP는 연결이 살아있는 줄 알고 60초 후에 쿼리 요청을 보냈다가 바로 `Communications link failure`라는 에러를 맞게 됩니다. 따라서 DB 서버와 라우터 등의 소켓 최대 대기시간을 파악하고, `max-lifetime`을 그보다 짧게 지정하여 선제적으로 커넥션을 교체해야 합니다.
    - `connection-timeout`은 Tomcat의 스레드가 커넥션 풀에게 커넥션 할당을 요청한 뒤 대기하는 최대 시간이며, `minimum-idle`은 최소 소켓 유지 수, `maximum-pool-size`는 최대 소켓 유지 수입니다.

### 🛠️ 문제 해결

#### 1. Free-Tier의 한계로 인한 동시 회원가입 오류 방어
- **문제 상황:** JMeter로 1000 Thread, 10초 Ramp-up으로 부하 테스트를 진행한 결과, Connection Timeout 에러율이 10%를 초과하는 병목 발생.
- **해결 과정:**
    - **CPU 병목 해소:** CPU 부하를 줄이기 위해 Spring Security의 `BCryptPasswordEncoder` 암호화 강도를 10(기본값)에서 4로 낮춤. ($2^{10}$번의 해시에서 $2^4$번의 해시로 연산량 대폭 감소)
    - **커넥션 풀 최적화:** 인프라의 강제 끊김 현상을 막기 위해 Hikari CP의 `max-lifetime`을 9분 40초(580000ms)로 설정함 (RDS의 `wait_timeout`이 600초임을 확인). 대기줄 정체를 막고 빠른 실패(Fail-fast)를 유도하기 위해 `connection-timeout`을 3000ms로 설정함.
    - **자원 할당:** 최소 커넥션 유지수인 `minimum-idle`을 10으로, 최대 커넥션 수인 `maximum-pool-size`를 50으로 설정 (RDS의 `max_connections` 값이 150임을 고려하여 DB의 물리적 부하 분산).
- **결과:**
    - 10초 동안 1000건의 회원가입 요청을 에러율 0%로 완벽하게 방어 완료 (약 100 TPS 달성).

| Label | #Sample | Average | Min | Max | Std.Dev. | Error% | Throughput | Received KB/sec | Sent KB/sec | Avg.Bytes |
|---| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 회원가입 | 1000 | 19 | 15 | 43 | 3.955 | 0.0 | 99.93 | 38.73 | 30.81 | 396.89 |
...
- SecurityContextHolder.getContext().getAuthentication().getPrincipal()은 Object 객체를 반환하며, 유효한 인증 정보가 없을 경우 순수 String 타입인 "anonymousUser"를 반환한다. (타입 캐스팅 시 주의 필요)
- Spring Boot 내장 Tomcat의 스레드(Thread)는 1개당 약 1MB의 메모리를 점유한다. 무작정 스레드 풀을 늘리는 것은 메모리(OOM) 한계에 부딪힐 수 있음을 인지했다.

### 🛠️ 문제 해결

#### 1. 다중 유저 동시성 테스트 시 '중복 좋아요' 예외 발생 논리적 오류 해결
- **문제 상황:** JMeter를 통해 서로 다른 100개의 고유 토큰으로 좋아요 API를 호출했으나, 첫 요청을 제외한 모든 요청이 "이미 좋아요를 누른 게시물입니다"라는 중복 에러를 발생시킴.
- **해결 과정:**
    - 디버깅 결과, 모든 요청의 유저 ID가 1로 고정되어 들어오는 현상 발견.
    - LoginUserArgumentResolver의 supportsParameter 로직에서 어노테이션 검사 메서드를 hasParameterAnnotation이 아닌 hasMethodAnnotation으로 잘못 작성한 오타를 발견함.
    - 이로 인해 리졸버가 무시(Bypass)되면서 비어있는 더미 MemberDto(id=1)가 컨트롤러로 계속 바인딩되어 발생한 문제였음. 관련, 코드를 올바르게 수정.
- **결과:**
    - 정상적으로 다중 유저(각기 다른 ID) 기반의 동시 좋아요 처리 로직 검증 완료.

#### 2. 톰캣과 HikariPool의 병목현상으로 인한 연결 미스 발생
- **문제 상황:** JMeter로 1,000 Threads, Ramp-up 1초(단 1초 만에 1,000건 폭격) 조건으로 부하 테스트 진행 시, 에러율 23.4%, 최고 응답 시간 5.2초 지연 및 java.net.SocketException: Connection reset 에러 발생.
- **해결 과정:**
    - Tomcat 병목: 1초 만에 몰린 트래픽을 기존 설정(400 threads, 300 accept-count)이 감당하지 못해 네트워크 연결이 거절됨. ➡️ 가용 메모리를 고려하여 max-threads 400, accept-count 500으로 튜닝.
    - DB 커넥션 고갈 (핵심 원인): 리졸버에서 유저 식별자(userId)를 얻기 위해 매 요청마다 memberRepository.findByEmail 쿼리를 발생시킴. 1,000개의 요청이 10개의 HikariCP 커넥션을 두고 경합하며 서버 자원이 고갈됨. ➡️ 로그인 시 JWT Payload 내부에 userId를 직접 포함시켜 발급하도록 수정. 리졸버에서의 DB I/O를 완전히 제거하여 완벽한 Stateless(무상태) 아키텍처로 개선함.
- **결과:**
    - 개선 후 동일 조건 테스트 시, 에러율 0.2%로 감소, 평균 응답속도 대폭 단축 및 초당 400 TPS 이상의 압도적인 성능 향상 달성.

| Label | #Sample | Average | Min | Max | Std.Dev. | Error% | Throughput | Received KB/sec | Sent KB/sec | Avg.Bytes |
|-------| --- | --- | --- | --- | --- |--------| --- | --- | --- | --- |
| 좋아요   | 1000 | 1027 | 13 | 2143 | 465.6489030063316 | 0.20   | 401.92926045016077 | 159.22247098573152 | 173.05488218448554 | 05.653 |
...
- 마이크로 서비스 환경이나 다중 서버 환경시 좋아요 스케줄러를 동시에 돌리면 정합성의 문제를 초래할 수 있는데, 그 해결법으로 Redisson이라는 Redis분산락을 이용할 수 있다.
- 처리 중의 결과를 처리하고 삭제하는 원자성을 보장하기 위해서, Atomic Swap Pattern을 이용할 수 있다.

### 🛠️ 문제 해결

#### 1. 스케줄러 오류시 데이터 유실 방지
- **문제:** Redis의 pop을 이용해 데이터를 꺼내서 처리할 경우, DB 저장 중 서버 전원이 나가는 Hard Crash 발생 시 꺼내둔 데이터가 허공으로 증발(Lost Update)하는 치명적 결함 발견.
- **해결:** 
  - 데이터를 1건씩 꺼내는 대신, Redis의 rename 명령어를 활용해 작업 대상 바구니의 이름을 순식간에 바꿔치기하는 Atomic Swap 패턴을 적용. 작업 중 서버가 죽더라도 PROCESSING_KEY에 데이터가 온전히 보존되어, 서버 재기동 시 실패한 지점부터 안전하게 재처리할 수 있는 멱등성(Idempotency)을 확보함. (동시에 Race Condition 원천 차단)

- **문제:** 이벤트 등으로 100만 건의 데이터가 쏟아졌을 때, members로 전체를 한 번에 조회하면 Redis 싱글 스레드가 블로킹되고 Spring 서버에 OOM이 발생할 수 있음.
- **해결:**
  - distinctRandomMembers를 활용하여 메모리에 최대 1,000건씩만 분할(Chunking)하여 적재하고, DB 저장이 완벽히 성공한 데이터만 remove하는 무한 루프 처리 방식을 도입하여 서버 리소스를 극도로 안정화함.

#### 2. 다중 서버 환경의 스케줄러 중복 실행 방어
- **문제:** 서버 인스턴스가 2대 이상으로 확장될 경우, 동일한 좋아요 동기화 스케줄러가 동시에 실행되어 DB 중복 Insert 및 데드락이 발생할 위험이 존재함.
- **해결:** Redisson을 활용해 스케줄러 진입점에 **분산락(Distributed Lock)**을 적용. 다른 서버가 이미 락을 점유 중이라면 대기 없이 즉시 포기(Wait Time 0)하도록 설계하여 스레드 낭비를 막고 단일 실행을 완벽히 보장함.

#### 테스트 결과

- Threads : 1000 / Ramps-up : 1

| Label | #Sample | Average | Min | Max | Std.Dev. | Error% | Throughput | Received KB/sec | Sent KB/sec | Avg.Bytes |
|-------| --- | --- | --- | --- | --- |--------| --- | --- | --- | --- |
| 좋아요   | 1000 | 1711 | 9 | 3570 | 1100.5665013982568 | 0.035  | 255.55839509327882 | 117.47724931318682 | 106.39290306350627 | 470.721 |

- Threads : 1000 / Ramps-up : 5

| Label | #Sample | Average | Min | Max | Std.Dev. | Error% | Throughput | Received KB/sec | Sent KB/sec | Avg.Bytes |
|-------| --- | --- |-----|  | --- |--------| --- | --- | --- | --- |
| 좋아요   | 1000 | 15 | 12   | 41 | 3.066965764399724 | 0.0  | 200.04000800160034 | 78.51042864822965 | 86.30202603020605 | 401.893 |
</div>